...
public class OpenFlowControlMessageProvider extends AbstractProvider
    ...
    private static final int AGGR_INIT_DELAY = 10;
    private static final int AGGR_PERIOD = 2;
    private static final TimeUnit AGGR_TIME_UNIT = TimeUnit.SECONDS;
    ...
    private class InternalDeviceProvider implements OpenFlowSwitchListener {
        ...
        public void switchAdded(Dpid dpid) {
            ...
            OpenFlowControlMessageAggregator ofcma =
                    new OpenFlowControlMessageAggregator(
    ...
    private class InternalControlMessageListener implements OpenFlowMessageListener {
        ...
        public void handleIncomingMessage(Dpid dpid, OFMessage msg) {
            if (msg.getType() == OFType.PACKET_IN ||
                ...
                /*
                if(msg.getType() == OFType.PACKET_IN){
                    OFPacketIn ofPacketIn = (OFPacketIn) msg;
                    //aggregators.get(dpid).getStartTimes().put(msg.getXid(),start);
                    log.info("incoming switch id"+dpid.toString()+"'s xid is"+msg.getXid()+"，bufferId"+ofPacketIn.getBufferId());
                }
                 */
            ...
            for (OFMessage msg : msgs) {
                if (msg.getType() == OFType.PACKET_OUT ||
                    ...
                    if(msg.getType() == OFType.FLOW_MOD){
                        ...
                        OpenFlowControlMessageAggregator aggregator = aggregators.get(dpid);
                        log.info("outcoming switch id"+dpid.toString()+"'s xid is"+msg.getXid()+"，bufferId"+ofFlowMod.getBufferId());

                        if(aggregator.getStartTimes().containsKey(msg.getXid())){
                            aggregator.addLatency(end-aggregator.getStartTimes().get(msg.getXid()));
                            log.info("match!"+msg.getType());
                            aggregators.get(dpid).getStartTimes().remove(msg.getXid());
                        }

                        aggregator.addLatency(end-msg.getXid());
